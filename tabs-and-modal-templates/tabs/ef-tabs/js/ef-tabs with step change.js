...
;( function( window ) {
	...
	efTabs.prototype._init = function() {
		...
		// Step tabs
		this.step = [].slice.call( this.el.querySelectorAll( '.content-wrap > section .steps .step' ) );
		// content items
	...
	efTabs.prototype._initEvents = function() {
		...
		this.step.forEach( function( step, idx ) {
		console.log(step);
	...
	efTabs.prototype._show = function( idx ) {
		if( this.current >= 0 ) {
			...
			this.step[ this.current ].className = this.items[ this.current ].className = '';
		...
		this.step[ this.current ].className = 'tab-current';
